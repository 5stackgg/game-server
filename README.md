...
```sh
dotnet build FiveStack.sln
dotnet test apps/swiftly/test/FiveStack.Tests.csproj
```

Images build from the repo root so they can reach `shared/`:

```sh
docker build -f apps/counterstrikesharp/Dockerfile .
```

`codepier up` hot-swaps into the CounterStrikeSharp dev pod; add
`--deployment dev-swiftly-game-server` for the SwiftlyS2 one. Both sync the repo root, because
the plugin projects compile sources from `shared/`.
