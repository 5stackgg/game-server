...
// What GET /utility/{id}/trajectory answers with.
//
...
// discover it moved.
public class UtilityTrajectoryArtifact
{
    public List<TrajectoryPoint> path { get; set; } = new List<TrajectoryPoint>();
    public SmokeVolume? smoke_volume { get; set; }

    public static UtilityTrajectoryArtifact Parse(byte[] body)
    {
        return Parse(PracticeJson.Text(body));
    }

    public static UtilityTrajectoryArtifact Parse(string body)
    {
        var artifact = new UtilityTrajectoryArtifact();
