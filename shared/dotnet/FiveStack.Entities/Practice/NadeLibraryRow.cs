...
// A row of GET /nades/library, exactly as the API returns it. The steps of a
...
// line itself needs a second call to GET /nades/{id}/trajectory.
public class NadeLibraryRow
{
    ...
    public LineupRecord ToLineup()
    {
        ...
        var lineup = new LineupRecord
        {
            ...
            name = name ?? "",
            map = map_name ?? "",
            utility_type = PracticeLineupUtility.NormalizeUtilityType(nade_type ?? ""),
            side = side ?? "TERRORIST",
            technique = technique ?? "",
