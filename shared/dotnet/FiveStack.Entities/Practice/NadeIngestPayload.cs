...
// The body of POST /nades/ingest, exactly as the API defines it.
//
...
public class NadeIngestPayload
{
    public string? match_id { get; set; }
    public string? author_steam_id { get; set; }
    public string? nade_type { get; set; }
    ...
    public List<NadePathPoint>? path { get; set; }

    public static NadeIngestPayload From(LineupRecord lineup)
    {
        return new NadeIngestPayload
        {
            author_steam_id = Text(lineup.author_steam_id),
            nade_type = PracticeLineupUtility.NormalizeUtilityType(lineup.utility_type),
            ...
            path = lineup
                .trajectory.Select(point => new NadePathPoint
                {
                    tick = point.t,
                    x = point.p.x,
                    y = point.p.y,
