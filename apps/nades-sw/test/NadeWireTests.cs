...
public class NadeWireTests
{
    ...
    {
        NadeIngestPayload payload = NadeIngestPayload.From(Lineup());

        Assert.Equal("76561198000000001", payload.author_steam_id);
        Assert.Equal("Smoke", payload.nade_type);
    ...
    {
        Assert.Equal(expected, NadeIngestPayload.MillisecondsFromSeconds(seconds));
    }
    ...
    {
        Assert.Equal(63, NadeIngestPayload.MillisecondsFromSeconds(0.0625f));
    }
    ...
    public void TheNadeTypeIsSentInTheApisSpelling(string recorded, string expected)
    {
        LineupRecord lineup = Lineup();
        lineup.utility_type = recorded;

        Assert.Equal(expected, NadeIngestPayload.From(lineup).nade_type);
    }
    ...
    private static NadeLibraryRow Row()
    {
        return new NadeLibraryRow
        {
            id = "panel-id",
            name = "Window smoke",
            map_name = "de_mirage",
    ...
    private static NadeLibraryRow Seeded(string? confidence = null)
    {
        NadeLibraryRow row = Row();
    ...
    {
        LineupRecord original = Lineup();
        NadeIngestPayload payload = NadeIngestPayload.From(original);

        var row = new NadeLibraryRow
        {
            id = "panel-id",
            name = payload.name,
            nade_type = payload.nade_type,
            side = payload.side,
