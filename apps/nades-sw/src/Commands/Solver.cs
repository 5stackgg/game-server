...
{
    [Command("nade_solver_calibrate", registerRaw: true, permission: "")]
    ...
    [Command("nade_solver_solve", registerRaw: true, permission: "")]
    public void OnSolverSolve(ICommandContext context)
    {
        ...
        if (!PracticeSolverUtility.TryParse(context.Args, out SolveRequest request, out string error))
        {
            ...
            Reply(
                context,
                $" {ChatColors.Grey}usage: nade_solver_solve target=x,y,z [from=x,y,z] [utility=Smoke] [steam=id] [name=...] [tolerance=40] [grenades=300] [seconds=120]"
            );
            return;
        }
