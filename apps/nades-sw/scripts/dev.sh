...
# reloads the plugin when NadePractice.dll changes.
#
# Chain: dotnet watch build -> apps/nades-sw/src/build/net10.0 -> cp -> /opt/dev -> SwiftlyS2 reload

log() { echo "[dev.sh $(date '+%H:%M:%S')] $*"; }

PROJECT="/opt/5stack/apps/nades-sw/src/NadePractice.csproj"
BUILD_OUTPUT="/opt/5stack/apps/nades-sw/src/build/net10.0"
...
PLUGIN_MOUNT="/opt/instance/game/csgo/addons/swiftlys2/plugins/NadePractice"
if mountpoint -q "$PLUGIN_MOUNT" 2>/dev/null; then
  DEV_DIR="$PLUGIN_MOUNT"
else
  DEV_DIR="/opt/dev/nades-sw"
  mkdir -p "$DEV_DIR"
fi
...
sync_to_dev() {
  ...
  if cp -r "$BUILD_OUTPUT"/. "$DEV_DIR"/ 2>/tmp/dev-cp.err; then
    if [ -f "$DEV_DIR/NadePractice.dll" ]; then
      log "synced -> $DEV_DIR (NadePractice.dll $(stat -c%s "$DEV_DIR/NadePractice.dll" 2>/dev/null || echo '?') bytes)"
    else
      log "WARNING: copied but $DEV_DIR/NadePractice.dll is missing"
    fi
  else
