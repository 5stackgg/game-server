...
{
    private static NadePlaybookStep Step(
        int order,
        int offsetMs,
    ...
    {
        return new NadePlaybookStep
        {
            nade_lineup_id = $"lineup-{order}",
            step_order = order,
            offset_ms = offsetMs,
    ...
    private static NadePlaybook Playbook(params NadePlaybookStep[] steps)
    {
        return new NadePlaybook
        {
            id = "book",
            name = "A execute",
            map_name = "de_mirage",
    ...
    {
        Assert.Equal(0, PlaybookUtility.DurationMs(new List<NadePlaybookStep>()));
        Assert.Equal(
            2500,
    ...
    {
        NadePlaybookStep step = Step(1, 0);
    ...
    {
        NadePlaybookStep step = Step(1, 0, assigned: " 76561198000000001 ");
