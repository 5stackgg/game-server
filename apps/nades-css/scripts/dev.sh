...
dotnet build apps/nades-css/src

dotnet watch build --project apps/nades-css/src &
dotnet_watch_pid=$!

# Set up trap to kill dotnet watch process on script exit
trap kill_dotnet_watch EXIT

directory_to_watch="/opt/5stack/apps/nades-css/src/bin/Debug/net10.0"

# Matches the path apps/counterstrikesharp/scripts/setup.sh symlinks in.
DEV_DIR="/opt/dev/nades-css"
mkdir -p "$DEV_DIR"
