...
public class MatchEvents
{
    ...
    {
        // Always log the state change, even offline (where we don't send it).
        _logger.LogInformation(
            "PublishMapStatus status={Status} winning_lineup_id={WinningLineupId}{Offline}",
            status,
            winningLineupId?.ToString() ?? "<null>",
            _environmentService.IsOfflineMode() ? " (offline, not sent)" : ""
        );
    ...
    ) GetRoundInformation()
    {
        ...
        if (match == null || matchData == null || currentMap == null)
        {
            ...
            return (0, 0, Team.None, Team.None, 0);
        }
        ...
        Team lineup2Side = TeamUtility.GetLineupSide(
            matchData,
            currentMap,
