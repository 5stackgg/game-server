...
public class GameDemos
{
    ...
    public void Start()
    {
        ...
        _gameServer.Message(MessageType.Alert, _localizer["demos.recording"]);

        Directory.CreateDirectory(GetMatchDemoPath());

        string demoPath =
            $"{GetMatchDemoPath()}/{MatchUtility.GetSafeMatchPrefix(match)}_{DateTime.Now.ToString("yyyyMMdd-HHmm")}-{_core.Engine.GlobalVars.MapName}";
