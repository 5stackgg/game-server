...
for ((i=from_num; i<=to_num; i++)); do
  ...
  image="ghcr.io/5stackgg/swiftly-game-server"
  tag="v0.0.$i"

  image_id=$(gh api \
    -H "Accept: application/vnd.github+json" \
    -H "X-GitHub-Api-Version: 2022-11-28" \
    "/orgs/5stackgg/packages/container/swiftly-game-server/versions" \
  ...
  if [[ -n "$image_id" ]]; then
    ...
    gh api -X DELETE \
      -H "Accept: application/vnd.github+json" \
      -H "X-GitHub-Api-Version: 2022-11-28" \
      "/orgs/5stackgg/packages/container/swiftly-game-server/versions/$image_id" &
  else
