...
PLUGIN_MOUNT="/opt/instance/game/csgo/addons/swiftlys2/plugins/UtilityPractice"
if mountpoint -q "$PLUGIN_MOUNT" 2>/dev/null; then
  DEV_DIR="$PLUGIN_MOUNT"
else
  DEV_DIR="/opt/dev/utility-sw"
  mkdir -p "$DEV_DIR"
fi
...
sync_to_dev() {
  ...
  if cp -r "$BUILD_OUTPUT"/. "$DEV_DIR"/ 2>/tmp/dev-cp.err; then
    if [ -f "$DEV_DIR/UtilityPractice.dll" ]; then
      log "synced -> $DEV_DIR (UtilityPractice.dll $(stat -c%s "$DEV_DIR/UtilityPractice.dll" 2>/dev/null || echo '?') bytes)"
    else
      log "WARNING: copied but $DEV_DIR/UtilityPractice.dll is missing"
    fi
  else
